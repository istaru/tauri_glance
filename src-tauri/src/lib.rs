...
// Windows overlay 用：人类可读格式，带一位小数和 /s 后缀，与设计稿一致
#[cfg(target_os = "windows")]
fn format_speed_win(bps: f64) -> String {
    let kb = 1024.0f64;
    let mb = kb * 1024.0;
    let gb = mb * 1024.0;
    if bps < kb       { format!("{:.0} B/s", bps) }
    else if bps < mb  { format!("{:.1} K/s", bps / kb) }
    else if bps < gb  { format!("{:.1} M/s", bps / mb) }
    else              { format!("{:.1} G/s", bps / gb) }
}

// macOS / Linux 托盘图标用：紧凑像素格式（与 SpeedFormatter.swift 一致）
...
fn start_monitor(app: AppHandle) {
    std::thread::spawn(move || {
        loop {
            ...
            // 格式与设计稿一致：CPU/内存整数，网速带一位小数和 /s 后缀
            #[cfg(target_os = "windows")]
            {
                let line1 = format!("CPU: {}  ↑: {}", m.cpu_percent.min(99), format_speed_win(m.upload_bps));
                let line2 = format!("内存: {}  ↓: {}", m.mem_percent.min(99), format_speed_win(m.download_bps));
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                .always_on_top(true)
                .skip_taskbar(true)
                .inner_size(160.0, 36.0)  // 与 macOS 菜单栏图标尺寸匹配
                .resizable(false)
                .visible(false)  // 先隐藏，定位完成后再显示，避免位置跳变
                .build()?;

                // 获取主显示器逻辑分辨率，贴紧任务栏底部（窗口高 36px，任务栏通常 40px，居中对齐）
                let screen_h = overlay
                    ...
                    .unwrap_or(1080.0);
                let _ = overlay.set_position(tauri::LogicalPosition::new(0.0_f64, screen_h - 38.0));
